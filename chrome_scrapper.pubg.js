...
const url_game = 'https://dapubg.com/#faq';


const table_prefix = "pubg_";
    ...
    const runNewScrapperPage = async (browser, db, callback) => {
        ...
        await page.exposeFunction('saveDoubleRound', (round) => {
            if(!page_removed){
                ...
                db.collection(table_prefix+'rounds').update({"roundId": round['roundId']}, round, { upsert: true })
        ...
        await page.exposeFunction('saveCrashRound', (round) => {
            if(!page_removed){
                ...
                db.collection(table_prefix+"crash").update({"roundId": round['roundId']}, round, { upsert: true })
                .then(function(){}).catch(function(err){
...
function insertDoubleRound(connection, round) {
    ...
    INSERT INTO ${table_prefix}double_round(
        roundId,
        hash,
        salt,
        roundTime,
...
function insertDoubleRoundUser(connection, user) {
    var deferred = q.defer(); 
    var query = connection.query(`
    INSERT INTO ${table_prefix}double_round_user(
        roundId,
        roundBetColor,
        playerId,
        playerName,
...
function insertCrashRound(connection, round) {
    var deferred = q.defer(); 
    var query = connection.query(`
    INSERT INTO ${table_prefix}crash_round(
        roundId,
        hash,
        salt,
        roundTime,
...
function insertCrashRoundUser(connection, user) {
    var deferred = q.defer(); 
    var query = connection.query(`
    INSERT INTO ${table_prefix}crash_round_user(
        roundId,
        roundNumber,
        playerId,
        playerName,

    ...
    const runNewScrapperPage = async (browser, db, callback) => {
        ...
        await page.exposeFunction('saveDoubleRound', (round) => {
            if(!page_removed){
                ...
                .then(function(){ 
                    logger.info(`saveDoubleRound MYSQL; roundId - ${round['roundId']};`); 
                },
                function(err){
                ...
                if(round.users){
                    for (let user of round.users) {
                        ...
                        .then(function(){ 
                            logger.info(`saveDoubleRound MYSQL; playerId - ${user['playerId']};`); 
                        },
                        function(err){
                            logger.info(`saveDoubleRound MYSQL; ERROR playerId - ${user['playerId']}; ${err};`);
                        })
                    }
                } 
            }
        ...
        await page.exposeFunction('saveCrashRound', (round) => {
            if(!page_removed){
                ...
                .then(function(){ 
                    logger.info(`saveCrashRound MYSQL; roundId - ${round['roundId']};`); 
                },
                function(err){
                ...
                if(round.users){
                    for (let user of round.users) {
                        ...
                        .then(function(){ 
                            logger.info(`saveCrashRound MYSQL; playerId - ${user['playerId']};`); 
                        },
                        function(err){
                            logger.info(`saveCrashRound MYSQL; ERROR playerId - ${user['playerId']}; ${err};`);
                        })
                    }
                }
